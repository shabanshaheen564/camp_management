...
<div class="map-wrapper position-relative">
    ...
    <div id="map">
        <div class="map-legend-box">
            ...
            <div class="ml-item">
                <div class="ml-sq" style="background:transparent;border:2px dashed #1d4ed8;width:14px;height:11px"></div> حدود منطقة الدراسة
            </div>
            <div class="ml-item">
...
<script>
    ...
    if (campBounds.length) map.fitBounds(campBounds, {
        padding: [40, 40]
    });
    ...
    function analyzeLandSuitability() {
        if (!geojsonData.length) {
            alert('يرجى رفع ملف الـ Shapefile أولاً');
            return;
        }
            ...
            CAMPS_DATA.forEach(camp => {
                ...
                features.forEach(feat => {
                    ...
                    if (pointInPolygon([lng, lat], feat.geometry)) {
                        if (gc === 1) insideBest = true;
                        else insideGood = true;
                    }
    ...
    async function analyzeRoutes() {
        ...
        if (!hosps.length) {
            alert('أضف مستشفى واحد على الأقل');
            return;
        }
    ...
    async function deleteHospital(id) {
        ...
        try {
            const res = await fetch(`/map/hospitals/${id}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': CSRF
                }
            ...
            if (hospitalMarkers[id]) {
                map.removeLayer(hospitalMarkers[id]);
    ...
    async function analyzeRelocation() {
        if (!uploadedShapefiles.length) {
            alert('يرجى رفع ملف الـ Shapefile أولاً من تبويب "الطبقات"');
            return;
        }
        ...
        features.forEach(feat => {
            ...
            candidatePolygons.push({
                ...
                cy,
                nearestHosp,
                hospDist: minHospDist,
                overlapsExistingCamp,
                score
            });
        });
...
<div><i class="fas fa-ruler-combined" style="color:#2563eb;width:16px"></i> مساحة الأرض: ${cand.areaDonum.toFixed(2)} دونم</div>
            ...
            campResults.forEach(result => {
                ...
                const campLat = +result.camp.latitude,
                    campLng = +result.camp.longitude;
                const arrow = L.polyline(
                    [
                        [campLat, campLng],
                        [result.bestCandidate.cy, result.bestCandidate.cx]
                    ], {
                        color: '#d97706',
                        weight: 3,
                        opacity: 0.85,
